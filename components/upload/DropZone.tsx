    ...
    <div
      ...
      className={`relative border-2 border-dashed rounded-2xl p-8 text-center transition-all cursor-pointer select-none ${
        dragging
          ...
          : 'border-[#1a2d4a] bg-[rgba(255,255,255,0.01)] hover:border-[#3b82f6]/40 hover:bg-[#3b82f6]/[0.03]'
      }`}
    >
      <input
        id={`input-${label}`}
      ...
      {loading ? (
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-[13px] text-[#3b82f6] font-medium">Procesando...</p>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-2">
          <div className={`w-10 h-10 rounded-[12px] flex items-center justify-center ${
            dragging ? 'bg-[#3b82f6]' : 'bg-[rgba(255,255,255,0.04)]'
          } transition-colors`}>
            <svg width="18" height="18" fill={dragging ? 'white' : '#6b85a8'} viewBox="0 0 20 20">
          ...
          <div>
            <p className="text-[13px] font-medium text-[#f0f4ff]">
              {fileName ?? 'Arrastrá el archivo o hacé clic para seleccionar'}
            </p>
            {fileName && <p className="text-[12px] text-[#3b82f6] mt-0.5 truncate max-w-xs mx-auto">{fileName}</p>}
            {hint && <p className="mt-1.5 text-[12px] text-[#6b85a8]">{hint}</p>}
          </div>
        </div>
      )}
    </div>
