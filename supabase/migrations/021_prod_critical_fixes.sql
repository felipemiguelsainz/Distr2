-- ============================================================
-- Fixes críticos para producción (auditorías seguridad + DB)
-- ============================================================

-- ---- SEC-1 / R1: RLS con scope en resumen_clientes_pdv -------
-- Hoy `rcp_read_all` deja leer toda la tabla a cualquier rol.
-- Reemplazar por policies con el mismo scope que resumen_diario.
...
DO $$ BEGIN
  ALTER TABLE config_meses ADD CONSTRAINT config_meses_mes_chk
    CHECK (mes BETWEEN 1 AND 12);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  ALTER TABLE config_meses ADD CONSTRAINT config_meses_dias_chk
    CHECK (dias_laborables BETWEEN 1 AND 31);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- ---- C1: CHECK de no-negatividad en metas --------------------
DO $$ BEGIN
  ALTER TABLE metas ADD CONSTRAINT metas_kilos_chk
    CHECK (kilos_meta >= 0);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  ALTER TABLE metas ADD CONSTRAINT metas_neto_chk
    CHECK (neto_meta IS NULL OR neto_meta >= 0);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- ---- D1: lock en recalcular_resumen_diario -------------------
-- Evita race condition si dos uploads concurrentes tocan fechas
-- solapadas (DELETE + INSERT sin lock veían estado parcial).
...
-- recalcular_resumen_clientes_pdv: mismo lock defensivo
