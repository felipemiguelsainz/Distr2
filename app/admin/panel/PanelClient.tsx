...
export function PanelClient({ meses }: { meses: MesConfig[] }) {
  ...
  const [values, setValues] = useState<Record<string, number>>(
    Object.fromEntries(meses.map(m => [`${m.anio}-${m.mes}`, m.dias_laborables]))
  );
              ...
              >
                ...
                <div className="flex items-center gap-2.5">
                  <input
                    ...
                    className="w-16 px-2.5 py-[7px] text-[13px] text-center font-semibold bg-[rgba(255,255,255,0.02)] border border-[#1a2d4a] rounded-[8px] text-[#f0f4ff] caret-[#3b82f6] focus:outline-none focus:border-[rgba(99,102,241,0.4)] transition-all"
                  />
